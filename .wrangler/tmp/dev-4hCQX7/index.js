...
var _McpAgent = class _McpAgent2 extends DurableObject2 {
  ...
  static serveSSE(path, {
    ...
    return {
      fetch: /* @__PURE__ */ __name(async (request, env3, ctx) => {
        const corsResponse = handleCORS(request, corsOptions);
        if (corsResponse) return corsResponse;
        const url = new URL(request.url);
        const namespace = env3[binding2];
...
var env22 = getEnv();
console.log(`env2: ${JSON.stringify(env22)}`);
  ...
  async init() {
    ...
    this.server.tool(
      ...
      async ({ query }) => {
        ...
        const APIkey = await env2.GIPHY_API_KEY.get();
...
var src_default = {
  fetch(request, env3, ctx) {
    ...
    if (url.pathname === "/sse" || url.pathname === "/sse/message") {
      return MyMCP.serveSSE("/sse").fetch(request, env3, ctx);
    }
    if (url.pathname === "/mcp") {
      return MyMCP.serve("/mcp").fetch(request, env3, ctx);
    }
    return new Response("Not found", { status: 404 });
  }
};
...
// .wrangler/tmp/bundle-W4NuDB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-W4NuDB/middleware-loader.entry.ts
